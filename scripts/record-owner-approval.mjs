...
const target = approvalTargets.find((approvalTarget) => approvalTarget.issue === args.issue);
...
if (!target) {
  printRows([
    ...
    ["Known issues", approvalTargets.map((approvalTarget) => `#${approvalTarget.issue}`).join(", ")]
  ]);
  console.log("");
  console.log("ok owner approval recorder ready");
  console.log("blocked approval recording requires known --issue");
...
const approvalComments = comments.filter((comment) => comment.body?.includes(target.exactApprovalText));
...
if (!args.write) {
  ...
  console.log(`npm run owner:record-approval -- --issue ${target.issue} --write --confirm-exact "${target.exactApprovalText}"`);
  console.log("");
  console.log("ok owner approval recorder ready");
...
function printUsage() {
  ...
  console.log("  npm run owner:record-approval -- --issue 255 --write --confirm-exact \"<exact approval text>\"");
}
