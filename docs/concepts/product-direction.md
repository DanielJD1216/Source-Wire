...
| Private local CLI | Story 6.1 offline configuration, Story 6.2 memory-only two-tool composition, Story 6.3 immutable synthetic-provider four-tool composition, Story 6.4 fail-closed orchestration and cleanup, and Story 6.5 explicit database status plus migration control |
...
The local Alpha proves that the core lifecycle can operate across real processes and disposable PostgreSQL. Story 6.2 proves the memory-only path can be composed behind one private local command without giving the MCP child owner or database authority. Story 6.3 proves the same command can bind one immutable repository synthetic provider while keeping provider selection and authority outside caller input. Story 6.4 proves invalid startup, dependency outage, child crash, credential invalidation, protocol separation, and cleanup fail closed in both synthetic provider paths. Story 6.5 proves read-only runtime-role database status, safe migration planning, explicit apply, exact migrator authority, transaction rollback, and idempotency. It does not prove an external or live provider and does not convert the repository into a hosted, deployed, managed-database, or production-ready product.
