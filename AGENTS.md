...
- `@source-wire/local-runtime@0.1.0-alpha.1` is deprecated and must not be recommended. Unpublished `0.1.0-alpha.2` is the current security-fix candidate. Its package exports remain the root composition API and two synthetic provider proof entrypoints. Do not import package internals. Candidate preparation does not authorize publication, production, hosting, deployment, real data, live providers, Windows, HTTP or SSE MCP, static serving, or non-disposable databases.
