...
const tempRoot = await mkdtemp(join(tmpdir(), "source-wire-runtime-boundary-installed-"));
...
try {
  ...
  const smokeResult = await runChecked(
    process.execPath,
    ["examples/runtime-boundary/synthetic-boundary-smoke.mjs"],
    installedPackageRoot
  );

  console.log(`ok runtime boundary installed smoke ${pack.name}@${pack.version}`);
  console.log(smokeResult.stdout.trim());
  console.log("ok installed runtime boundary example");
} finally {
