...
- latest-source `apps/alpha1-runtime` workspace and unpublished candidate for disposable PostgreSQL bootstrap, credential lifecycle, authenticated health, a four-tool stdio MCP surface, pending candidates, owner-controlled approval or rejection, audited active trusted-memory search, owner correction and revocation, canonical export, fresh portable initialization, isolated physical recovery, and protected synthetic source-evidence reads,
...
- deprecated public npm Alpha Story 6.8 snapshot and unpublished Story 6.9 security candidate with curated exports, package-content checks, exact advisory disposition, and local packed-consumer proof,
...
- locally pack and evaluate the unpublished `0.1.0-alpha.2` candidate from an explicit reviewed revision,
...
deprecated. Its registry tags do not make it a supported version. Unpublished
`0.1.0-alpha.2` requires a new exact release approval after review and hosted
verification.
