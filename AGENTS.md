...
This file is the repository entrypoint for AI coding agents. The published `@source-wire/contracts@0.2.0` package is the current immutable contracts snapshot and includes `KnowledgeProvider v1`. Public npm Alpha `@source-wire/local-runtime@0.1.0-alpha.2` is the reviewed security-fix runtime for macOS and Linux local evaluation with synthetic or disposable data. Its predecessor, `0.1.0-alpha.1`, is deprecated. Local-runtime Git tags, GitHub releases, deployment, hosting, production, real data, and live providers remain blocked.
...
- `@source-wire/local-runtime@0.1.0-alpha.1` is deprecated and must not be recommended. Published `0.1.0-alpha.2` is the current reviewed security-fix Alpha. Its package exports remain the root composition API and two synthetic provider proof entrypoints. Do not import package internals. npm availability does not authorize production, hosting, deployment, real data, live providers, Windows, HTTP or SSE MCP, static serving, non-disposable databases, a Git tag, or a GitHub release.
- Provider adapters are trusted in-process application code. The host must bind actor owner and requested namespace before invocation, enforce a hard response deadline, provide cooperative cancellation where supported, and use the shared protected handoff for both search and exact fetch.
