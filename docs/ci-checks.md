...
| Hosted runtime child issue publication preflight | `ok hosted runtime child issue publication preflight ready`, `ok child issue publication packet current`, `ok child issue publisher dry run current`, `ok child issue publisher guard smoke current`, `ok runtime readiness gate current`, `ok runtime proof intake gate current`, `ok child issue approval status current`, `ok owner open issue boundary current`, `ok owner open issue future planning smoke current`, `blocked hosted runtime child issue publication pending owner approval`, `blocked hosted runtime implementation` | The owner-side publication preflight verifies the child issue packet, dry-run publisher, guarded write smoke, runtime readiness, runtime proof intake, approval status, and open-issue boundary while publication and runtime implementation remain blocked. |
