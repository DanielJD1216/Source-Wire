...
const blockedScope = [
  ...
  "production or real-data trusted-memory correction, revocation, export, and recovery",
...
for (const requiredPath of [
  ...
  "docs/getting-started/alpha1-story5-knowledge-provider-runtime-host.md",
...
printList([
  ...
  "alpha1:conformance:story1 uses Node.js 22.23.1 plus generated disposable authoritative PostgreSQL 18.4 state, with PostgreSQL 16 available only through explicit compatibility selection, to prove the live loopback Story 1 path, stalled-body deadline recovery, and cleanup",
  "alpha1:conformance:story2 uses a real MCP client, loopback API, owner CLI, and generated disposable authoritative PostgreSQL 18.4 state, with PostgreSQL 16 available only through explicit compatibility selection, to prove pending proposal, owner decision, durable lifecycle idempotency, atomic audit, least privilege, and cleanup",
  "alpha1:conformance:story3 uses real API and MCP processes plus generated disposable authoritative PostgreSQL 18.4 state, with PostgreSQL 16 available only through explicit compatibility selection, to prove audited active-memory search, exact origin-process receipts, fail-closed crashes and outages, bounds, leak resistance, least privilege, and cleanup",
  "alpha1:conformance:story4 runs 27 cases using real API, owner CLI, operator CLI, local CLI, and generated disposable authoritative PostgreSQL 18.4 state, with PostgreSQL 16 available only through explicit compatibility selection, to prove correction, revocation, protected-read races, deterministic secret-free export, Story 6.6 exact-owner local export and overwrite policy, fresh portable initialization, isolated physical recovery, runtime verification gates, least privilege, and cleanup",
  "alpha1:conformance:story5 runs 30 real-process and disposable PostgreSQL cases for immutable provider binding, protected evidence release, deadline enforcement, unified search and exact-fetch handoff, Story 6.3 composition, and Story 6.4 fail-closed orchestration plus cleanup",
...
printList([
  ...
  "runtime:database-implementation-packet validates the historical synthetic database-posture unit; the separately bounded Alpha 1 Stories 1 through 5 workspace is the only latest-source migration and database path",
...
printList([
  "AGENTS.md distinguishes the published contracts package, synthetic examples, and unpublished local Alpha 1 Stories 1 through 5 proof for AI agents",
  ...
  "docs/getting-started/alpha1-story3-audited-search.md defines the unpublished Story 3 active-only FTS, audit-before-release receipt, crash, outage, leak, and cleanup proof",
  "docs/getting-started/alpha1-story4-governed-lifecycle-portability.md defines the unpublished Story 4 correction, revocation, protected-read race, canonical export, portable initialization, physical recovery, runtime verification, and cleanup proof",
  "docs/getting-started/alpha1-story5-knowledge-provider-runtime-host.md defines the unpublished Story 5 immutable synthetic provider, source-evidence tools, protected release, conformance, advisory, cleanup, and blocked scope",
